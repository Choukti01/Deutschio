...
async function sendVerification(email: string, url: URL) { if (resend) await resend.emails.send({ from: env.EMAIL_FROM, to: email, subject: "Verify your Deutschio account", html: `<h2>Welcome to Deutschio</h2><p>Verify your email to begin learning:</p><p><a href="${url}">Verify email</a></p>` }); }
...
app.post("/api/v1/auth/signup", authLimiter, async (req, res, next) => {
  try {
    ...
    const user = (await query<UserRow>(`INSERT INTO users (id, email, password_hash, verification_token_hash, verification_token_expires_at) VALUES ($1,$2,$3,$4,$5) RETURNING ${userColumns}`, [id(), input.email, await bcrypt.hash(input.password, 12), crypto.createHash("sha256").update(rawToken).digest("hex"), new Date(Date.now() + 86_400_000)])).rows[0];
...
app.post("/api/v1/auth/login", authLimiter, async (req, res, next) => {
  try {
    ...
    if (!user || !await bcrypt.compare(input.password, user.password_hash)) return res.status(401).json({ error: { code: "INVALID_CREDENTIALS", message: "Email or password is incorrect" } });
...
start().catch(async (error) => { console.error("Failed to start Deutschio API", error); await pool.end(); process.exit(1); });
